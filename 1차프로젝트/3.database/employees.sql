...
db.employees.find({number:1002, department:'sales'})
...
db.employees.insertOne({number:1005 , last_name:'박', first_name:'둘리', salary:55000 , department:'clerk', status: 'B'})

//24.select * from employees whe6e status = 'A'
...
db.employees.updateOne({last_name:'박'},{$set:{salary: 55010}})
//38.update employees set salary = salary + 100 where number in (1005,1006)
db.employees.updateMany({number:{$in:[1005, 1006]}},{$inc:{salary:10}})
