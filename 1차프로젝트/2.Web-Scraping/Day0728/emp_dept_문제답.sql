...
select empno,job,deptno
  from emp
where empno not in (select distinct mgr from emp);# 매니저에 속해 있지 않다면 그건 바로 말단사원
...
# " 함수명(널값을 확인할 컬럼(널아니면 출력될 컬럼),널값일경우 대체하여 데이터를 보여줄 컬럼(혹은값)) "
